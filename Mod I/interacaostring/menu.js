...
function main(){
    ...
    menu += '\n3 - Upgrade de plano'
    menu += '\n5 - Palavras com letras predefinidas'
    menu += '\n\n0 - Sair'
...
function uses_only(palavras){
    const letrasobrigatorias = input('Letras exigidas: ')
    let contador = 0
    for (let palavra of palavras){
        if (temletrasobrigatorias(palavra,letrasobrigatorias)){
            console.log(palavra)
            contador++
        }
    }
    const percentual = (contador / palavras.length) * 100
   console.log(`Existem ${contador} (${percentual.toFixed(3)}%) com as letras "${letter}"`) 
}

function temletrasobrigatorias(palavra, letrasobrigatorias){
    for (let letras of palavra){
        if (letras === letrasobrigatorias){
            return true
        }
    } return ('erro')
